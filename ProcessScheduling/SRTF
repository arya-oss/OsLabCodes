/************************************************
 * title: SRTF.cpp
 * Description: Shortest Remaining Time First Scheduling. (No priority)
...
int main() {
    ...
    while(!ready_queue.empty()) {
        ...
        p->rt--;
        schedulerTime++;
        while(offset < N && P[offset].at == schedulerTime){ // check which process arrived at that time
            ready_queue.push(P+offset);
            offset++;
        }
        if(p->rt == 0){ // process completed
            p->ct = schedulerTime;
            p->tat = p->ct - p->at;
            p->wt = p->tat - p->bt;
        }
